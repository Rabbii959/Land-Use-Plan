...
**Map view** — all 16,980 parcels on a Leaflet canvas, in eight thematic layers,
over a street, satellite or plain basemap. Every parcel is clickable and reports
its class, category, area in acres and km², and its class totals.

**Cross-filtering** — the share ribbon, the layer list, the category switch, the
charts and the table are all wired to the same filter. Selecting a bar, a ribbon
band or a table row isolates that class on the map and recomputes the statistics
panel; selecting a class also zooms the map to its extent.
...
**Areas here are computed from the polygon geometry, not read from the source
file's `Area_SqKm` attribute.** That attribute could not be used:

- It returns **zero for 11 of the 60 classes**, including every urban block —
  all 3,539 Residential Urban Block parcels, all 1,567 List A Abutting parcels,
  and every Commercial and Industrial Urban Block record.
- Its units are not square kilometres. Summing it gives 10,434,340, which is
  four orders of magnitude away from any plausible reading of the district.

Each polygon's area is instead computed by spherical excess on the WGS 84
...
```bash
SRC=Hafizabad.json OUT=. python3 tools/build_data.py
```
...
bounding box, vertex reduction and per-file sizes — so any change to the source
can be checked against the figures above.

To retune the map layers, edit `tools/groups.json`, which maps each of the 60
land use classes to one of the eight thematic groups, and re-run.
...
16,980 polygons across 60 land use classes, attributed by land use, category
(existing or proposed), and name where recorded.
